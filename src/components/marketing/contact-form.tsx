...
import { contactSchema, inquiryTypes, type ContactInput } from "@/lib/validation";
...
export function ContactForm({ defaultType }: { defaultType?: ContactInput["type"] }) {
    ...
    return (
      <div className="border border-ink/10 bg-paper p-10 text-center">
        ...
        <p className="mt-3 text-sm leading-relaxed text-ink/60">
          Your concierge inquiry has been received. We will be in touch
          shortly to arrange your private consultation.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-7" noValidate>
      <div>
        <label htmlFor="type" className="mb-2 block text-[0.7rem] uppercase tracking-widest2 text-ink/45">
          Inquiry Type
        </label>
        <select id="type" {...register("type")} className={cx(inputClasses, "appearance-none")}>
          {inquiryTypes.map((t) => (
      ...
      <div className="grid gap-7 sm:grid-cols-2">
        ...
        <div>
          <label htmlFor="company" className="mb-2 block text-[0.7rem] uppercase tracking-widest2 text-ink/45">
            Company <span className="text-ink/30">(optional)</span>
          </label>
          <input id="company" {...register("company")} className={inputClasses} placeholder="For corporate or physician inquiries" />
        </div>
      </div>
